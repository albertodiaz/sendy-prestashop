...
class Sendynewsletter extends Module
{
	...
	{
		Db::getInstance()->execute('DROP TABLE '._DB_PREFIX_.'sendynewsletter');
	...
	public function getContent()
	{
		...
		elseif (Tools::isSubmit('subscribedmerged'))
		{
			...
			{
				$id = (int)substr($id, 1);
				$sql = 'UPDATE '._DB_PREFIX_.'sendynewsletter SET active = 0 WHERE id = '.$id;
	...
	public function getSubscribers()
	{
		...
		$dbquery->from('sendynewsletter', 'n');
	...
	protected function unregister($email, $register_status)
	{
		if ($register_status == self::GUEST_REGISTERED)
			$sql = 'DELETE FROM '._DB_PREFIX_.'sendynewsletter WHERE `email` = \''.pSQL($_POST['email']).'\' AND id_shop = '.$this->context->shop->id;
	...
	{
		$this->context->controller->addCSS($this->_path.'sendynewsletter.css', 'all');
		$this->context->controller->addJS($this->_path.'sendynewsletter.js');
	}
	...
	{
		//if e-mail of the created user address has already been added to the newsletter through the sendynewsletter module,
		//we delete it from sendynewsletter table to prevent duplicates
		$id_shop = $params['newCustomer']->id_shop;
		$email = $params['newCustomer']->email;
		if (Validate::isEmail($email))
			return (bool)Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'sendynewsletter WHERE id_shop='.(int)$id_shop.' AND email=\''.pSQL($email)."'");
